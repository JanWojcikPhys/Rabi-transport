...
function ewolucja(d,tmax)                               # tmax - ostatni czasowy element 
    ...
    for t in 1:tmax
        ...
        for y in 0:d-1
            if y<d/2
                psi[y*2+1:y*2+2] = [c (-s)*exp(1im*pi/6);s*exp(-1im*pi/6) c]./sqrt(1) * psi[y*2+1:y*2+2]
            else
                psi[y*2+1:y*2+2] = [c2 s2*exp(1im*pi/6);(-s2)*exp(-1im*pi/6) c2]./sqrt(1) * psi[y*2+1:y*2+2]
            end
        end
...
npzwrite("fig5v1.npy",prob[151,:])
prob2 = Matrix(ewolucja2(301,151));
npzwrite("fig5v2.npy",prob2[151,:])